...
# Initialize session state to store inventory, item count, and day counter
...
def add_item(name, quantity, purchase_quantity, shelf_life_days, cost_per_unit, day):
    ...
    item = {
        ...
        "Day Added": day,  # Track the day the item was added
    }
    st.session_state.inventory.append(item)

# Function to issue material from inventory
def issue_material(item_id, issue_quantity, day):
    for item in st.session_state.inventory:
        if item["Item ID"] == item_id:
            if item["Balance Quantity"] >= issue_quantity:
                ...
                item["Day Issued"] = day  # Track the day the item was issued
                st.success(f"Issued {issue_quantity} units of '{item['Name']}' on Day {day}.")
            else:
                st.error(f"Insufficient stock for '{item['Name']}'. Available: {item['Balance Quantity']}")

# Function to calculate inventory status for a specific day
def calculate_inventory_status(selected_day):
    updated_inventory = []
    for item in st.session_state.inventory:
        # Check if the item was added on or before the selected day
        if item["Day Added"] <= selected_day:
            # Check if the item was issued on or before the selected day
            if "Day Issued" in item and item["Day Issued"] <= selected_day:
                balance_quantity = item["Balance Quantity"]
            else:
                balance_quantity = item["Initial Quantity"] + item["Purchase Quantity"]
            
            # Check if the item has expired by the selected day
...
with st.sidebar:
    st.header("Inward")
    day = st.number_input("Day", min_value=1, step=1, value=st.session_state.current_day)
    ...
    for i in range(num_items):
        ...
        if st.button(f"Add Item {i + 1}"):
            if name:  # Check if the name is not empty
                add_item(name, quantity, purchase_quantity, shelf_life_days, cost_per_unit, day)
                st.success(f"Item '{name}' added to inventory on Day {day}!")
            else:
...
with st.sidebar:
    ...
    if st.session_state.inventory:
        day = st.number_input("Day", min_value=1, step=1, value=st.session_state.current_day, key="issue_day")
        ...
        if st.button("Issue"):
            ...
            issue_material(selected_item_id, issue_quantity, day)

# Select day to view inventory status
selected_day = st.number_input("Select Day to View Inventory Status", min_value=1, step=1, value=st.session_state.current_day)

# Calculate inventory status for the selected day
updated_inventory = calculate_inventory_status(selected_day)

# Display the inventory as of the selected day
st.header(f"Status of Inventory on Day {selected_day}")
